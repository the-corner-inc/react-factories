/**
 * Base UI sidebar primitives for the intranet shell.
 *
 * Framework-agnostic port for the registry: no @radix-ui, no cva, no button
 * primitive. Uses the registry shims (`#/components/ui/sheet`) and `#/lib/utils`.
 */
...
/** Matches shadcn's default mobile breakpoint (below 768px). */
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    if (openProp !== undefined || restoredRef.current) return;
    restoredRef.current = true;
    const entry = document.cookie
      .split("; ")
      .find((cookie) => cookie.startsWith(`${SIDEBAR_COOKIE_NAME}=`));
    if (entry) {
      const stored = entry.split("=")[1] === "true";
      _setOpen(stored);
    }
  }, [openProp]);
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex h-8 w-8 items-center justify-center gap-2 rounded-lg text-sm font-medium whitespace-nowrap text-sidebar-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-3 focus-visible:ring-sidebar-ring outline-none disabled:pointer-events-none disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
        className,
      )}
      onClick={(event) => {
        onClick?.(event);
...
const sidebarMenuButtonVariant: Record<"default" | "outline", string> = {
...
const sidebarMenuButtonSize: Record<"default" | "sm" | "lg", string> = {
  default: "h-8 text-sm",
  sm: "h-7 text-xs",
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          ...
          sidebarMenuButtonVariant[variant],
          sidebarMenuButtonSize[size],
          className,
        ),
      },
      props,
